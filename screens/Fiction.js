...
const Fiction = ({ route }) => {
  // Extract the data passed from the navigation
...
const navigation=useNavigation();
  ...
  useEffect(() => {
    Animated.timing(marginTopAnimation, {
      toValue: 20, // Target value for marginTop
      duration: 1000, // Animation duration
      easing: Easing.ease, // Easing function
      useNativeDriver: false, // To use native driver for performance
  ...
  return (
    <ScrollView style={styles.container}>
      <Animated.View style={[styles.welcomeContainer, { marginTop: marginTopAnimation }]}>
        <Text style={styles.welcomeText}>Welcome to Fiction Section</Text>
      </Animated.View>
      <View style={styles.section}>
        <ScrollView horizontal>
          <TouchableOpacity style={[styles.category, styles.selectedCategory]}>
            <Text style={[styles.categoryText, styles.selectedCategoryText]}>Fiction</Text>
          </TouchableOpacity>
          <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  selectedCategory: {
    backgroundColor: "#333", // Dark color for selected category
  },
  categoryText: {
    fontSize: 16,
  },
  selectedCategoryText: {
    color: "#fff", // Text color for selected category
  },
  bookContainer: {
    flexDirection: "row",
    marginBottom: 20,
